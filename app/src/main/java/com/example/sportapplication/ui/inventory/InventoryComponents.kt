...
fun ItemCatalogueUI(
    ...
    Column(
        ...
        if(isOpen){Column{
            ...
            Text("Description:")
            if(item.itemEffectOnXp != null){
                Text("The item increase XP collected from quests by: ${item.itemEffectOnXp}")
            }
            if(item.itemEffectOnDuration!= null){
                Text("The item increases the duration of events by: ${item.itemEffectOnDuration}")
            }
            Spacer(
                modifier = Modifier
                    .fillMaxWidth()
